...
class FeedbackController extends Controller
{
    ...
    public function store(Request $request, $performerId)
    {
        ...
        // Check if the performer portfolio exists
        if (!$performer) {
        ...
        $completedBooking = Booking::where('client_id', $userId)
            ...
            ->where('status', 'Accepted') // Assuming 'completed' is the status for a finished booking
            ->exists();
        ...
        $feedback->performer_id = $performerId; // Correctly link to performer_id
    ...
    public function canLeaveReview($performerId)
    {
        ...
        $completedBooking = Booking::where('client_id', $userId)
            ->where('performer_id', $performerId)
            ->where('status', 'Accepted') // Assuming 'APPROVED' is the status for a finished booking
            ->exists();
